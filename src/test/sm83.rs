...
use crate::gameboy::cpu::regs::RegisterFile;
...
macro_rules! cpu_test {
    ($testfn:ident, $instr:expr, $cycle_count_correct:expr) => {
        ...
        fn $testfn() {
            ...
            let filename = format!("tests/sm83-test-data/cpu_tests/v1/{:02x}.json", $instr);
            let testcases: Value =
...
fn run_testcase(testcase: &Value, cycle_count_correct: bool) {
    ...
    // verification later.
    let bus = std::mem::replace(&mut cpu.bus, Box::new(Testbus::new()));
    let bus = bus.downcast::<Testbus>().unwrap();
